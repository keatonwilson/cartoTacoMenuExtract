...
from src.scraping import scout_spot, find_duplicates, discover_candidates
...
st.markdown(
    "Research taco spots we haven't visited yet. Scouted spots stage as "
    "**pending (unvetted)** entries — they appear on the map flagged as pending "
    "until the team visits and runs the normal menu-photo pipeline."
)

CONFIDENCE_ICONS = {"high": "🟢", "medium": "🟡", "low": "🔴"}

tab_scout, tab_discover = st.tabs(["🔭 Scout one spot", "🗺️ Discover candidates"])


# ════════════════════════════════════════════════════════════════════
# Tab 1: Scout a single known spot (name-first deep research)
# ════════════════════════════════════════════════════════════════════
with tab_scout:
    ...
    if "scout_result" in st.session_state:
        ...
        sub_site, sub_hours, sub_desc = st.tabs(["Site Info", "Hours", "Description"])
...
# ════════════════════════════════════════════════════════════════════
# Tab 2: Discover candidates (city-wide search, diffed against what we track)
# ════════════════════════════════════════════════════════════════════
with tab_discover:
    st.markdown(
        "One web-search pass finds spots we don't track yet, diffed against "
        "production and staging. Tick the promising ones — each selected "
        "candidate gets a full scout run and stages as a pending spot for "
        "review in **Staging Review**."
    )

    c1, c2 = st.columns([1, 2])
    disc_limit = c1.slider("Max candidates", 5, 30, 20)
    disc_focus = c2.text_input(
        "Focus (optional)", placeholder="e.g. south side, birria, food trucks", key="disc_focus"
    )

    if st.button("🗺️ Discover candidates", type="primary"):
        with st.spinner("Searching Tucson for spots we don't have yet..."):
            try:
                candidates, _raw = discover_candidates(disc_limit, disc_focus)
                st.session_state["disc_candidates"] = [c.model_dump() for c in candidates]
                st.session_state.pop("disc_results", None)
                # Clear stale checkbox state from a previous discovery run
                for key in list(st.session_state.keys()):
                    if key.startswith("disc_sel_"):
                        del st.session_state[key]
                st.rerun()
            except Exception as e:
                st.error(f"Discovery failed: {e}")

    if "disc_candidates" in st.session_state:
        candidates = st.session_state["disc_candidates"]
        new_count = sum(1 for c in candidates if not c["already_known"])
        st.divider()
        st.markdown(
            f"**{len(candidates)} candidates** — {new_count} look new, "
            f"{len(candidates) - new_count} match spots we already track."
        )

        selected_idx = []
        for i, c in enumerate(candidates):
            col_check, col_info = st.columns([0.5, 6])
            checked = col_check.checkbox(
                "Select", value=not c["already_known"], key=f"disc_sel_{i}",
                label_visibility="collapsed", disabled=c["already_known"],
            )
            bits = [f"**{c['name']}**"]
            if c.get("area"):
                bits.append(c["area"])
            ...
            if c.get("url"):
                line += f" · [source]({c['url']})"
            if c["already_known"]:
                line += " · ✓ *already tracked*"
            col_info.markdown(line)
            if checked and not c["already_known"]:
                selected_idx.append(i)

        st.divider()
        st.caption(
            "Each selected candidate runs one full web-research pass (a few "
            "candidates per minute), then auto-geocodes and stages unless the "
            "deep scout reveals a duplicate."
        )
        if st.button(
            f"🔭 Scout & stage {len(selected_idx)} selected",
            type="primary",
            ...
            for done, i in enumerate(selected_idx):
                c = candidates[i]
                status_line.markdown(f"Scouting **{c['name']}**… ({done + 1}/{len(selected_idx)})")
                try:
                    spot, raw = scout_spot(c["name"], [c["url"]] if c.get("url") else None)

                    # Auto-geocode so the proximity dedup below has coordinates
                    if spot.site.address and not (spot.site.lat_1 and spot.site.lon_1):
                        coords = geocode_address(spot.site.address)
                        if coords:
                            spot.site.lat_1, spot.site.lon_1 = coords

                    # The deep scout may reveal it's a duplicate the name diff missed
                    dupes = find_duplicates(
                        spot.restaurant_name, spot.site.lat_1, spot.site.lon_1
                    )
                    if dupes["production"]:
                        match = dupes["production"][0]
                        results.append({
                            "name": c["name"],
                            "outcome": f"⏭️ skipped — matches {match['name']} (est_id {match['est_id']}, {match['match']})",
                        })
                        continue

                    row_id = save_scraped_spot(spot, raw)
                    geo = "📍 geocoded" if spot.site.lat_1 else "⚠️ no coordinates — geocode in review"
                    results.append({"name": c["name"], "outcome": f"✅ staged ({geo}) — `{row_id}`"})
                except Exception as e:
                    results.append({"name": c["name"], "outcome": f"❌ failed: {e}"})
                progress.progress((done + 1) / len(selected_idx))
    ...
    if "disc_results" in st.session_state:
        ...
        for r in st.session_state["disc_results"]:
            st.markdown(f"- **{r['name']}**: {r['outcome']}")
        staged_count = sum(1 for r in st.session_state["disc_results"] if r["outcome"].startswith("✅"))
        if staged_count:
            st.success(
                f"{staged_count} spot(s) staged as pending. Next: review and approve them in "
                "**📋 Staging Review** (filter: Scouted spots), then promote in **🚀 Promote**."
            )
